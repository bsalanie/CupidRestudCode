...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#+++  Artelys Knitro 11.0.0 Python API
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
#---- KTR_callback
_KTR_callback64 = ctypes.CFUNCTYPE(
    ctypes.c_int,
    ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_longlong, ctypes.c_longlong,
...
class _KTR_callback_wrapper (object):
    def __init__ (self, kc, fnPtr):
        ...
        self._fnPtr = fnPtr
        sig_handler = _signal_handler (self.call)
        self.c_fnPtr = _KTR_callback64 (sig_handler.call)
    def set_KTR_context_ptr (self, kc):
        self._kc = kc
    def call (
        self,
...
#---- KTR_lsq_callback
_KTR_lsq_callback64 = ctypes.CFUNCTYPE(
    ctypes.c_int,
...
class _KTR_lsq_callback_wrapper (object):
    def __init__ (self, kc, fnPtr):
        ...
        self._fnPtr = fnPtr
        sig_handler = _signal_handler (self.call)
        self.c_fnPtr = _KTR_lsq_callback64 (sig_handler.call)
    def set_KTR_context_ptr (self, kc):
        self._kc = kc
    def call (
        self,
...
#---- KTR_newpt_callback
_KTR_newpt_callback64 = ctypes.CFUNCTYPE(
    ctypes.c_int,
    KTR_context_ptr,
    ctypes.c_int, ctypes.c_int, ctypes.c_longlong,
    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
...
class _KTR_newpt_callback_wrapper (object):
    def __init__ (self, kc, fnPtr):
        ...
        self._fnPtr = fnPtr
        sig_handler = _signal_handler (self.call)
        self.c_fnPtr = _KTR_newpt_callback64 (sig_handler.call)
    def set_KTR_context_ptr (self, kc):
        self._kc = kc
    def call (
        self,
...
#---- KTR_init_problem
_knitro.KTR_init_problem64.argtypes = [
    KTR_context_ptr,
    ctypes.c_int,
    ctypes.c_int, ctypes.c_int,
    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
    ctypes.c_int, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
    ctypes.c_longlong, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int),
    ctypes.c_longlong, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int),
    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double)
]
_knitro.KTR_init_problem64.restype = ctypes.c_int
def KTR_init_problem (
    ...
    return _knitro.KTR_init_problem64 (
        kc,
        n,
        objGoal,
        objType,
...
_knitro.KTR_lsq_init_problem64.argtypes = [
    KTR_context_ptr,
    ctypes.c_int,
    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
...
_knitro.KTR_lsq_init_problem64.restype = ctypes.c_int
def KTR_lsq_init_problem (
    ...
    if lambdaInitial is not None and n+m != len (lambdaInitial):
        raise ValueError ("Knitro-Python error: Array lambdaInitial has invalid non-zero size (must be exactly n+m)!")
    return _knitro.KTR_lsq_init_problem64 (
        kc,
        n,
        _cDoubleArray (xLoBnds), _cDoubleArray (xUpBnds),
        m, _cIntArray (rType),
...
_knitro.KTR_mip_init_problem64.argtypes = [
    KTR_context_ptr,
    ctypes.c_int,
    ctypes.c_int, ctypes.c_int, ctypes.c_int,
...
_knitro.KTR_mip_init_problem64.restype = ctypes.c_int
def KTR_mip_init_problem (
    ...
    if lambdaInitial is not None and len (lambdaInitial) > 0 and  n+m != len (lambdaInitial):
        raise ValueError ("Knitro-Python error: Array lambdaInitial has invalid non-zero size (must be exactly n+m)!")
    return _knitro.KTR_mip_init_problem64 (
        kc,
        n,
        objGoal, objType, objFnType,
...
_knitro.KTR_set_func_callback64.argtypes = [KTR_context_ptr, _KTR_callback64]
_knitro.KTR_set_func_callback64.restype = ctypes.c_int
def KTR_set_func_callback (kc, fnPtr):
    ...
    _registerCallback (kc, fnPtrWrapper)
    return _knitro.KTR_set_func_callback64 (kc, fnPtrWrapper.c_fnPtr)

#---- KTR_set_grad_callback
_knitro.KTR_set_grad_callback64.argtypes = [KTR_context_ptr, _KTR_callback64]
_knitro.KTR_set_grad_callback64.restype = ctypes.c_int
def KTR_set_grad_callback (kc, fnPtr):
    ...
    fnPtrWrapper = _KTR_callback_wrapper (kc, fnPtr)
    _registerCallback (kc, fnPtrWrapper)
    return _knitro.KTR_set_grad_callback64 (kc, fnPtrWrapper.c_fnPtr)

#---- KTR_set_hess_callback
_knitro.KTR_set_hess_callback64.argtypes = [KTR_context_ptr, _KTR_callback64]
_knitro.KTR_set_hess_callback64.restype = ctypes.c_int
def KTR_set_hess_callback (kc, fnPtr):
    ...
    fnPtrWrapper = _KTR_callback_wrapper (kc, fnPtr)
    _registerCallback (kc, fnPtrWrapper)
    return _knitro.KTR_set_hess_callback64 (kc, fnPtrWrapper.c_fnPtr)

#---- KTR_lsq_set_res_callback
_knitro.KTR_lsq_set_res_callback64.argtypes = [KTR_context_ptr, _KTR_lsq_callback64]
_knitro.KTR_lsq_set_res_callback64.restype = ctypes.c_int
def KTR_lsq_set_res_callback (kc, fnPtr):
    ...
    _registerCallback (kc, fnPtrWrapper)
    return _knitro.KTR_lsq_set_res_callback64 (kc, fnPtrWrapper.c_fnPtr)

#---- KTR_lsq_set_jac_callback
_knitro.KTR_lsq_set_jac_callback64.argtypes = [KTR_context_ptr, _KTR_lsq_callback64]
_knitro.KTR_lsq_set_jac_callback64.restype = ctypes.c_int
def KTR_lsq_set_jac_callback (kc, fnPtr):
    ...
    fnPtrWrapper = _KTR_lsq_callback_wrapper (kc, fnPtr)
    _registerCallback (kc, fnPtrWrapper)
    return _knitro.KTR_lsq_set_jac_callback64 (kc, fnPtrWrapper.c_fnPtr)

#---- KTR_set_newpoint_callback
_knitro.KTR_set_newpt_callback64.argtypes = [KTR_context_ptr, _KTR_newpt_callback64]
_knitro.KTR_set_newpt_callback64.restype = ctypes.c_int
def KTR_set_newpt_callback (kc, fnPtr):
    ...
    _registerCallback (kc, fnPtrWrapper)
    return _knitro.KTR_set_newpt_callback64 (kc, fnPtrWrapper.c_fnPtr)
...
def KTR_set_newpoint_callback (kc, fnPtr):
    ...
    def fnPtr (evalRequestCode, n, m, nnzJ, nnzH, x, lambda_, obj, c, \
              objGrad, jac, hessian, hessVector, userParams)
    '''
    return KTR_set_newpt_callback (kc, fnPtr)

#---- KTR_set_ms_process_callback
_knitro.KTR_set_ms_process_callback64.argtypes = [KTR_context_ptr, _KTR_callback64]
_knitro.KTR_set_ms_process_callback64.restype = ctypes.c_int
def KTR_set_ms_process_callback (kc, fnPtr):
    ...
    fnPtrWrapper = _KTR_callback_wrapper (kc, fnPtr)
    _registerCallback (kc, fnPtrWrapper)
    return _knitro.KTR_set_ms_process_callback64 (kc, fnPtrWrapper.c_fnPtr)

#---- KTR_set_mip_node_callback
_knitro.KTR_set_mip_node_callback64.argtypes = [KTR_context_ptr, _KTR_callback64]
_knitro.KTR_set_mip_node_callback64.restype = ctypes.c_int
def KTR_set_mip_node_callback (kc, fnPtr):
    ...
    fnPtrWrapper = _KTR_callback_wrapper (kc, fnPtr)
    _registerCallback (kc, fnPtrWrapper)
    return _knitro.KTR_set_mip_node_callback64 (kc, fnPtrWrapper.c_fnPtr)
...
KTR_RC_UNBOUNDED_OR_INFEAS      = -301
...
KTR_LINSOLVER_MA97                  = 7
KTR_LINSOLVER_MA86                  = 8
...
KTR_BAR_SWITCHRULE_AUTO             = -1
KTR_BAR_SWITCHRULE_NEVER            = 0
...
KTR_PARAM_OUTNAME               = 1105
KTR_PARAM_OUT_CSVNAME           = 1106
KTR_PARAM_ACT_PARAMETRIC        = 1107
KTR_ACT_PARAMETRIC_NO               = 0
KTR_ACT_PARAMETRIC_MAYBE            = 1
KTR_ACT_PARAMETRIC_YES              = 2
KTR_PARAM_ACT_LPDUMPMPS         = 1108
KTR_ACT_LPDUMPMPS_NO                = 0
KTR_ACT_LPDUMPMPS_YES               = 1
KTR_PARAM_ACT_LPALG             = 1109
KTR_ACT_LPALG_DEFAULT               = 0
KTR_ACT_LPALG_PRIMAL                = 1
KTR_ACT_LPALG_DUAL                  = 2
KTR_ACT_LPALG_BARRIER               = 3
KTR_PARAM_ACT_LPPRESOLVE        = 1110
KTR_ACT_LPPRESOLVE_OFF              = 0
KTR_ACT_LPPRESOLVE_ON               = 1
KTR_PARAM_ACT_LPPENALTY         = 1111
KTR_ACT_LPPENALTY_AUTO              = 0
KTR_ACT_LPPENALTY_ALL               = 1
KTR_ACT_LPPENALTY_NONLINEAR         = 2
KTR_ACT_LPPENALTY_DYNAMIC           = 3
KTR_PARAM_CONVEX                = 1114
KTR_CONVEX_AUTO                     = -1
KTR_CONVEX_NO                       = 0
KTR_CONVEX_YES                      = 1
KTR_PARAM_OUT_HINTS             = 1115
KTR_OUT_HINTS_NO                    = 0
KTR_OUT_HINTS_YES                   = 1
KN_PARAM_BAR_MAXCORRECTORS      = 1117
KTR_PARAM_STRAT_WARM_START      = 1118
KTR_STRAT_WARM_START_NO             = 0
KTR_STRAT_WARM_START_YES            = 1
KTR_PARAM_FINDIFF_TERMINATE     = 1119
KTR_FINDIFF_TERMINATE_NONE          = 0
KTR_FINDIFF_TERMINATE_ERREST        = 1
KTR_PARAM_CPUPLATFORM           = 1120
KTR_CPUPLATFORM_AUTO                = -1
KTR_CPUPLATFORM_COMPATIBLE          = 1
KTR_CPUPLATFORM_SSE2                = 2
KTR_CPUPLATFORM_AVX                 = 3
KTR_CPUPLATFORM_AVX2                = 4
KTR_CPUPLATFORM_AVX512              = 5  # EXPERIMENTAL
KTR_PARAM_PRESOLVE_PASSES       = 1121
KTR_PARAM_PRESOLVE_LEVEL        = 1122
KTR_PRESOLVE_LEVEL_AUTO             = -1
KTR_PRESOLVE_LEVEL_1                = 1
KTR_PRESOLVE_LEVEL_2                = 2
KTR_PARAM_FINDIFF_RELSTEPSIZE   = 1123
KTR_PARAM_INFEASTOL_ITERS       = 1124
KTR_PARAM_PRESOLVEOP_TIGHTEN    = 1125
KTR_PRESOLVEOP_TIGHTEN_AUTO         = -1
KTR_PRESOLVEOP_TIGHTEN_NONE         = 0
KTR_PRESOLVEOP_TIGHTEN_VARBND       = 1
...
KTR_MIP_HEURISTIC_AUTO              = -1
KTR_MIP_HEURISTIC_NONE              = 0
...
KTR_PARAM_MIP_SELECTDIR         = 2034
KTR_MIP_SELECTDIR_DOWN              = 0
KTR_MIP_SELECTDIR_UP                = 1
KTR_PARAM_MIP_CUTFACTOR         = 2035
KTR_PARAM_MIP_ZEROHALF          = 2036
KTR_MIP_ZEROHALF_NONE               = 0
KTR_MIP_ZEROHALF_ROOT               = 1
KTR_MIP_ZEROHALF_TREE               = 2
KTR_MIP_ZEROHALF_ALL                = 3
KTR_PARAM_MIP_MIR               = 2037
KTR_MIP_MIR_AUTO                    = -1
KTR_MIP_MIR_NONE                    = 0
KTR_MIP_MIR_TREE                    = 1
KTR_MIP_MIR_NLP                     = 2
KTR_PARAM_MIP_CLIQUE            = 2038
KTR_MIP_CLIQUE_NONE                 = 0
KTR_MIP_CLIQUE_ROOT                 = 1
KTR_MIP_CLIQUE_TREE                 = 2
KTR_MIP_CLIQUE_ALL                  = 3
...
KTR_PARAM_PAR_CONICNUMTHREADS   = 3006